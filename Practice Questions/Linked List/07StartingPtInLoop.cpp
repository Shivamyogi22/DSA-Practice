...
Node* getStartingPoint(Node* &head){
    ...
    //Agr yha tk pauch gye hu mtlb slow aur fast meet kr gye h 
    slow = head; //-- slow ko head pr point krva do

    // Ab Slow and Fast 1-1 step chlenge
    while(fast != slow){
        slow = slow -> next;
        fast = fast -> next;
    }

    // return kr do starting ponint 
    return slow;
}
